...
void TIMG12_IRQHandler(void){
  if((TIMG12->CPU_INT.IIDX) == 1){ // this will acknowledge
    ...
    if (dash.getLane() == solderpoop.lane && solderpoop.vertPos <= 15 && solderpoop.vertPos >= 10 && dash.vertPos <= 10) dash.trip();
  ...
  while(!startGame) {
    ...
    switch(currentState){
      ...
      case gameState:
        ...
        else if (!buttons.pausePressed && lock) lock = false;
      ...
      case gameOver:
        ...
        if(buttons.pausePressed && !lock && gameOverCounter == 0 ){
          lock = true;
          gameOverCounter++;
          if(isSpanish){
            outlineButton(40, 59, 81, 30);
          }
          else{
            outlineButton(43, 59, 74, 30);
          }
        }
        else if(buttons.pausePressed && !lock && gameOverCounter == 1){
          lock = true;
          gameDone = false;
          gameEntered = false;
          startOutlined = 0;
